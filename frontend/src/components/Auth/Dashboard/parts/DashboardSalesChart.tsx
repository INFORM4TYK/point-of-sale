  ...
  const fetchOrders = async () => {
    try {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        ...
        allDates.push(date.toLocaleDateString("pl-PL", { day: "2-digit", month: "2-digit" }));
      }

      const salesMap: Record<string, number> = {};
      ordersWithItems.forEach((order) => {
        const date = new Date(order.created_at).toLocaleDateString("pl-PL", { day: "2-digit", month: "2-digit" });
        if (!salesMap[date]) salesMap[date] = 0;
        salesMap[date] += Number(order.total);
      });
